#!/bin/bash
set -e
echo "Running test with TextEditor5 project in $(pwd)"
echo "This tests placing jar file a couple of subdirectory levels deep, with leading dot in path"
if [ -z "$JDEPLOY" ]; then
...
echo "TextEditor 5 project test passed"
